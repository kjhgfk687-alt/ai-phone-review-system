...
| 🏠 型号查询主界面（知识库直取） | 📋 结构化结果与策展外观图 |
| --- | --- |
| ![主界面](docs/screenshots/home.png) | ![结果页](docs/screenshots/results.png) |

## 为什么做这个（背景与思路）

### 问题：选购决策的信息困境

项目启动前，我先做了一轮用户调研（问卷，N=56）验证需求是否真实存在，四个痛点全部得到数据支持：

- **参数看不懂**：55.4% 的用户无法完全自主看懂硬件参数（"完全看不懂"+"大部分概念模糊"+"细节容易混淆"）
- **评测不敢信**：53.6% 的用户认为现有评测带有偏向，需要多方对照才敢下结论
- **对比太耗时**：30.4% 的用户表示同价位机型太多、逐个对比成本高
- **噱头分不清**：25% 的用户分不清宣传话术和真实体验

其中"对比"是需求度最高的功能——83.9% 的用户表示比较需要或非常需要。这决定了产品以"结构化参数 + 横向对比"为核心卖点，面向三类人群：数码小白（~55%，要通俗解读和避坑提示）、爱好者（~45%，要快速提取和横向对比）、购机决策者（要客观的优缺点提炼）。

### 四个贯穿始终的设计决策

1. **客观可溯源**：只基于官网参数和知识库生成内容，评测句末强制标注【来源：xx】；检索资料与参数冲突时以参数为准——把"客观"从口号变成机制
2. **诚实优先**：参数"未提及"绝不推断为"不支持"；不存在的型号（如 X30000）诚实返回空而不是硬凑候选；官网确无渲染图时拒绝外观分析而不是拿海报凑数
3. **数据飞轮**：提取完成自动写回参数知识库，用户确认候选自动写回注册表——每次使用都让系统更聪明，78 台知识库即由此积累
4. **按数据规模选型**：551 块语料的体量下手写 BM25 + 向量混合检索，而非引入检索框架和向量数据库；全项目 API 成本控制在 5 元以内
...
### 混合检索（手写实现，零检索框架 / 零向量数据库）

- **分词**：字符二元组 + 一元组 + 整段直配（中文友好，无 jieba 依赖）
- **稀疏召回**：按 Okapi BM25 标准公式手写实现打分（k1=1.5, b=0.75，IDF 加权 + 长度归一）
...
| 检索 | 手写 BM25 打分与 RRF 融合 + bge-m3 稠密召回 + bge-reranker-v2-m3 重排（`rag.py`，无检索框架/无向量库） |
